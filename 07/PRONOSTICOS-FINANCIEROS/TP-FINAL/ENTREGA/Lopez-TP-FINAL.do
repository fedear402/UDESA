...
* el scheme de los graficos es uno que lo descargue de internet para que queden mas agradables
...
TEST DE TRAZA
H0 : r<=R
No rechazamos que sea como mucho 1 el rango
TEST DE MAXIMO AUTOVALOR
H0 : r = R
No rechazamos que el rango sea 1
*/
...
// Sin restricciones porque ninguna tiene tendencias
/*
TEST DE TRAZA
H0 : r<=R
 no rechazamos que sea como mucho 2 el rango
TEST DE MAXIMO AUTOVALOR
H0 : r = R
s no rechazamos que el rango sea 2
*/
...
twoway (line ln_edn fecha, lc(red)) ///
       (line ln_tran fecha, lc(green)) ///
       (line ln_cepu fecha, lc(pink)), ///
	   xlabel(, format(%dy) labsize(vsmall)) legend(off) ///
       title("Electricidad") ///
